...
class InspectionCP(Inspection):
		...
		def _licensesDataModification(self) -> None:
			...
			self.data.licensesDf = self.data.licensesDf[self.data.licensesDf.ACTIVITIE_TYPE_ID == 1] #Activity number for Health activities
			...
			#self.data.licensesDf.Latitude = self.data.licensesDf.Latitude[0:8]
			self.data.licensesDf.Latitude = pd.to_numeric(self.data.licensesDf.Latitude, errors='coerce')
			self.data.licensesDf.Longitude = pd.to_numeric(self.data.licensesDf.Longitude, errors='coerce')
			self.data.licensesDf = self.data.licensesDf.dropna(subset=['Latitude', 'Longitude'])

			# self.data.licensesDf.Latitude = self.data.licensesDf.Latitude.str[:12].astype('float')
			# self.data.licensesDf.Longitude = self.data.licensesDf.Longitude.str[:12].astype('float')
		...
		def _filterByInspectionType(self, censorship_type : str, activity_type_id : str) :
			...
			print('Filtered CRM cases:', self.data.crmData.shape)
		...
		def _initResultDf(self):
			...
			print("ALL COLUMNS",self.resultData.columns)
			config.meta_data=self.resultData[['CaseId', 'AMANACODE', 'AMANAARNAM', 'AMANAENAME',
			 	]]
		...
		def _getNumberOfPriorityAreasFeature(self):
			...
			self.priorityAreas.to_crs(epsg= 32637, inplace = True)
			#Calculating the number of priority areas for each case
			prarea = geopandas.sjoin(self.casesGeometryWithBuffer, self.priorityAreas, how = 'left', predicate = 'intersects')
		...
		def _getNumberOfLicensesInVicinityFeature(self):
			...
			retail_countdf = self.caseIdToLicenseId.groupby('CaseId')['LicenseId'].nunique().to_frame(name = 'no_of_health_licenses_vicinity').reset_index()
			# retail_countdf.head()
		...
		def _getAverageComplianceScore(self):
			...
			self.data.inspectionsDf['Degree of Compliance'] = self.data.inspectionsDf['Degree of Compliance'].astype('float')
		...
		def prepareFeatureDataset(self):
			...
			self.resultData.drop_duplicates(inplace = True)
			self.resultData.drop_duplicates(subset = ['CaseId'], inplace = True)
			print(self.resultData.columns)
			self.resultData['avg_compliance_percent'] = 100 - self.resultData['avg_compliance_percent']
			featuresColumns = ['DN','satisfaction_level','avg_compliance_percent', 'days_elapsed','no_of_priority_areas','no_of_health_licenses_vicinity','days_elapsed_last_inspection']
		...
		def _outputToFile(self, name = 'common'):
			...
			print('Feature records:', self.resultData.shape)
