...
class Excavation(InspectionRBD):
    ...
    def buildArtificialDf(self):
        ...
        self.resultDf= self.resultDf[ self.resultDf['INSPECTION_DATE']>= self.resultDf['DIGGING_START_DATE']]
    ...
    def buildFeatureDf(self):
        ...
        self.resultDf['ISVIOLATION'] =  self.resultDf['ISVIOLATION'].apply(lambda x: 1 if x else 0 )
        
        self.resultDf[self.featuresColumns].fillna(method="bfill",inplace=True)
        self.resultDf[self.featuresColumns].fillna(method="ffill",inplace=True)   
        return self.resultDf