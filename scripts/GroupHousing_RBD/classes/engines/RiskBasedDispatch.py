...
class RiskBasedDispatch ():
    ...
    def predict_discrete(self, features : pd.DataFrame) -> pd.DataFrame:
    ...
    def trainTestModel(self, data : pd.DataFrame, features_columns : list, labels_column : list) -> None:
        ...
        xgb_predict_df = self.predict_discrete(x_train)
        ...
        xgb_predict_df = self.predict_discrete(x_test)
    ...
    def trainTestModelGroupHousing(self, data : pd.DataFrame, features_columns : list, labels_column : list):
        self.trainTestModelExcavation(data, features_columns, labels_column, test_size = 0.3)

    def trainTestModelExcavation(self, data : pd.DataFrame, features_columns : list, labels_column : list, test_size = 0.33):
        ...
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, shuffle=True, stratify=y, random_state=21)
        ...
        X_test_transformed = self.scaler.transform(X_test.values)

        xgb_predict_df = self.predict_discrete(X_test_transformed)
        # print(confusion_matrix(y_test,xgb_predict_df['probability of violation']))
        # print(classification_report(y_test,xgb_predict_df['probability of violation']))
    ...
    def getModelResults(self, data, calculated_data : pd.DataFrame, inspectionType, licensesData : pd.DataFrame,) -> None:
        impact_weight = 0.5
    ...
    def getModelResultsExcavation(self, data, calculated_data : pd.DataFrame, inspectionType, licensesData : pd.DataFrame,) -> None:
    ...
    def getModelResultsConstruction(self, data, calculated_data : pd.DataFrame, inspectionType, licensesData : pd.DataFrame,) -> None:
    ...
    def getModelResultsGroupHousing(self, data, calculated_data : pd.DataFrame, inspectionType, licensesData : pd.DataFrame,) -> None:
        scaler = MinMaxScaler()

        predictions_df = calculated_data.copy()
        predictions_df['ESTIMATED_CAPACITY_LOG']= np.log(predictions_df['ESTIMATED_CAPACITY'])
        predictions_df['ESTIMATED_CAPACITY_SCALED']= (predictions_df.groupby('MUNICIPALI')[['ESTIMATED_CAPACITY_LOG']].apply(lambda x : self.scale(x))) + 1
        # predictions_df['ESTIMATED_CAPACITY_SCALED'] = scaler.fit_transform(predictions_df[['ESTIMATED_CAPACITY']])
        predictions_df['TOTAL_RISK']= (predictions_df['probability of violation'])*(predictions_df['ESTIMATED_CAPACITY_SCALED'])
        predictions_df['TOTAL_RISK']= 100*(predictions_df.groupby('MUNICIPALI')[['TOTAL_RISK']].apply(lambda x : self.scale(x)))

        predictions_df['TOTAL_RISK'].fillna(0,inplace=True)
        predictions_df['probability of violation'].fillna(0,inplace=True)

        predictions_df.loc[predictions_df['probability of violation']>0.75, 'probability_of_violation_discrete']='high (75% - 100%)'
        predictions_df.loc[predictions_df['probability of violation']<=0.75, 'probability_of_violation_discrete']='medium (50% - 75%)'
        predictions_df.loc[predictions_df['probability of violation']<=0.5, 'probability_of_violation_discrete']='low (0% - 50%)'

        predictions_df.loc[predictions_df['TOTAL_RISK']>75, 'TOTAL_RISK_discrete']='high (75 - 100)'
        predictions_df.loc[predictions_df['TOTAL_RISK']<75, 'TOTAL_RISK_discrete']='medium (50 - 75)'
        predictions_df.loc[predictions_df['TOTAL_RISK']<=50,'TOTAL_RISK_discrete']='low (0 - 50)'

        predictions_df.columns = [i.upper().replace(" ", "_") for i in predictions_df.columns]
