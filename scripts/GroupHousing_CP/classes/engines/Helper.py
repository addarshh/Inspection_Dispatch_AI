...
class Helper(ABC):
    ...
    def Engine_Start_Metadata_Update(self):
        ...
        sql_query_time = """Insert into %s.MLMODEL_DETAILS (MODEL_NAME, MODEL_STATUS, MODEL_START_TIME) \
    ...
    def insert_df_Batchwise(self, df, tablename, batch_size):
        ...
        df_temp.head(1).to_sql(name='{}'.format(tablename.lower()), con=self.engine, if_exists='replace',
                               schema=config.DB['outputschema'],
                               dtype=dtypedict)
        ...
        for i in range(len(splits)):
            ...
            insert_sql = '''
                INSERT INTO "{}"."{}" ({}) VALUES ({})'''.format(config.DB['outputschema'], tablename, schema_str,
                                                                 num_str)
            # print(insert_sql)
