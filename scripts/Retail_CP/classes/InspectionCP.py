...
class InspectionCP(Inspection):
		...
		def _licensesDataModification(self) -> None:
			...
			# For Retails Inspection Type
			self.data.licensesDf = self.data.licensesDf[self.data.licensesDf.ACTIVITIE_TYPE_ID == 2] 
			...
			# Replacing comma
			self.data.licensesDf.Latitude = self.data.licensesDf.Latitude.str.replace(',', '.')
			self.data.licensesDf.Longitude = self.data.licensesDf.Longitude.str.replace(',', '.')
		...
		def _initResultDf(self):
			...
			config.meta_data=self.resultData[['CaseId','OBJECTID', 'REGION', 'AMANACODE', 'AMANAARNAM', 'AMANAENAME',
			 	'SHAPE_AREA', 'SHAPE_LEN','index_right']]
