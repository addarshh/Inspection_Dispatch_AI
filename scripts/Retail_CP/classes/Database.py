...
class Database(ABC):
    ...
    def getInspectionsData(self):
        ...
        sqlQuery = """SELECT LICENSE_NUMBER "LICENSE NUMBER" , INSEPECTION_ID "INSEPECTION ID" , INSPECTYPE_TYPE_ID "INSPECTYPE ID" ,\
    ...
    def getLicensesKeysData(self):
        sqlQuery = f"SELECT D_ACTIVITIES_ID, D_ACTIVITIES_NAME, IS_ENABLE, ACTIVITIE_TYPE_ID, \
    ...
    def getPopulationDataGdf(self):
        print("getPopulationDataGdf")
        # self.populationData = geopandas.read_file(path.join(path.dirname(__file__), config.PATHS['population_data']))
        self.populationData=pd.read_csv(config.population_grids_path,dtype={"amanacode":"str","municipalitycode":"str","regioncode":"str"})
        self.populationData=geopandas.GeoDataFrame(self.populationData , geometry=geopandas.GeoSeries.from_wkt(self.populationData.geometry, crs = 'epsg:4326'))
        self.populationData.columns = map(str.upper, self.populationData.columns)
        self.populationData = self.populationData.rename(columns={'GEOMETRY': 'geometry'})
        self.populationData = self.populationData.rename(columns={'GRIDUNIQUECODE': 'GridNumber'})
        self.populationData = self.populationData.rename(columns={'GRIDNAME': 'GridName', 'MUNICIPALITY': 'MUNICIPALI', 'MUNICIPALITYCODE': 'MUNICIPA_1'})

    def getPopulationDataGdf2(self):
        print("getPopulationDataGdf2")
        self.populationData=pd.read_csv(config.population_grids_path,dtype={"amanacode":"str","municipalitycode":"str","regioncode":"str"})
        self.populationData=geopandas.GeoDataFrame(self.populationData , geometry=geopandas.GeoSeries.from_wkt(self.populationData.geometry, crs = 'epsg:4326'))
        self.populationData.columns = map(str.upper, self.populationData.columns)
        self.populationData = self.populationData.rename(columns={'GEOMETRY': 'geometry'})
        self.populationData = self.populationData.rename(columns={'GRIDUNIQUECODE': 'GridNumber'})
        self.populationData = self.populationData.rename(
            columns={'GRIDNAME': 'GridName', 'MUNICIPALITY': 'MUNICIPALI', 'MUNICIPALITYCODE': 'MUNICIPA_1'})
    ...
    def getCrmCases(self):
        ...
        WHERE LATITUDE IS NOT NULL AND LONGITUDE IS NOT NULL AND SHORT_STATUS <> 'Close'\
    ...
    def getAmanaDataGdf(self):
        print("amana data")
        self.amanaDataGdf = pd.read_csv(config.amana_shp_path, dtype={"amanacode":"str"})
        self.amanaDataGdf =geopandas.GeoDataFrame(self.amanaDataGdf , geometry=geopandas.GeoSeries.from_wkt(self.amanaDataGdf.geometry, crs = 'epsg:4326'))
        self.amanaDataGdf.columns = map(str.upper, self.amanaDataGdf.columns)
        self.amanaDataGdf = self.amanaDataGdf.rename(columns={'GEOMETRY': 'geometry'})
