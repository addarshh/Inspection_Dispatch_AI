...
class InspectionRBD(Inspection):
		...
		def _inspectionDataModification(self):
			...
			#print(self.data.inspectionsDf.shape)
		...
		def _licensesDataModification(self):
			...
			logging.info("""CHECK:MERGE df1:licensesDf Shape: {}
    		and df2:licensesKeysDf Shape: {} LEFT on BUSINESS ACTIVITY right on D_ACTIVITIES_NAME""".format(self.data.licensesDf.shape, self.data.licensesKeysDf.shape))
			...
			""".format(self.data.licensesDf.shape))
		...
		def getIsFineIssuedFeature(self)-> None:
				...
				# self.resultDf['fine_issued'] = 0
				# self.resultDf['Issued fine amount'] = 0
		
		def getIsNonCompliantFeature(self) -> None:
				print(self.resultDf.columns)
				self.resultDf['Degree of Compliance'] = self.resultDf['Degree of Compliance'].astype(float)
		...
		def extractFeaturesFromDf(self):
				...
				self.resultDf = self.resultDf[['License ID (MOMRAH)', 'INSEPECTION ID', 'Latitude', 'Longitude'] + self.featuresColumns + self.labelsColumn]
