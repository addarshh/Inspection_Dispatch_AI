...
class Retail(InspectionRBD):
    ...
    def filterByInspectionType(self):
        ...
        self.data.licensesDf = self.data.licensesDf[self.data.licensesDf['ACTIVITIE_TYPE_ID']==2].copy()

    
    
    def buildFeatureDf(self) -> pd.DataFrame:
        self.filterByInspectionType()
        logging.info("""CHECK:MERGE function:buildFeatureDf df1:licensesDf Shape: {} 
        and df2:licensesDf Shape: {} INNER on LICENSE NUMBER
        """.format(self.data.licensesDf.shape, self.data.licensesDf.shape))
        ...
        logging.info("""CHECK:MERGE_OUTPUT df:inspectionsWithLicensesDf {}
        """.format(self.inspectionsWithLicensesDf.shape))
    ...
    def buildArtificialDf(self) -> pd.DataFrame:
        logging.info("""CHECK:MERGE df1:inspectionsDf Shape: {}
    	and df2:licensesDf Shape: {} LEFT on BUSINESS ACTIVITY""".format(self.data.inspectionsDf.shape, self.data.licensesDf.shape))
